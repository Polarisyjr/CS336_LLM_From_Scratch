    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    # 1) Initialize vocabulary with special tokens and byte-level tokens.
    vocab = init_vocab(special_tokens)
    # 2) Construct a regular expression to split the text
    delim = special_split_regex(special_tokens)
...
def init_vocab(special_tokens: list[str]) -> dict[int, bytes]:
    """
    Initialize a byte-level BPE vocabulary.

    Layout (stable IDs):
      0 .. len(special_tokens)-1        : special tokens (UTF-8 bytes, in given order)
      len(special_tokens) .. +255       : single-byte tokens 0x00..0xFF
    """
    vocab: dict[int, bytes] = {}

    # 1) Special tokens first (stable, predictable ids).
    for i, tok in enumerate(special_tokens):
        vocab[i] = tok.encode("utf-8")

    # 2) Then the 256 byte tokens.
...
def special_split_regex(special_tokens: list[str]) -> str | None:
    """
    Build the delimiter regex used to split text on special tokens,
    as described in the assignment: regex.split("|".join(...)).

    Returns:
        A regex pattern string like "<\\|endoftext\\|>|<\\|pad\\|>"
        or None if no special tokens.
    """
    if not special_tokens:
        return None

    escaped = [regex.escape(tok) for tok in special_tokens]
    # Sort longer tokens first to avoid prefix matching issues.
